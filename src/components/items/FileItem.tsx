...
function getFileIcon(fileName: string | null): { color: string; bg: string } {
    ...
    switch (ext) {
        case 'pdf': return { color: 'text-red-500', bg: 'bg-red-500/10' };
        case 'doc': case 'docx': return { color: 'text-blue-500', bg: 'bg-blue-500/10' };
        case 'xls': case 'xlsx': return { color: 'text-green-500', bg: 'bg-green-500/10' };
        case 'zip': case 'rar': case '7z': return { color: 'text-yellow-500', bg: 'bg-yellow-500/10' };
        ...
        <div className="group relative theme-card rounded-2xl p-5 transition-all duration-300 hover:shadow-lg">
            <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                    <h4 className="text-sm theme-text font-medium truncate">{item.file_name || 'Unknown file'}</h4>
